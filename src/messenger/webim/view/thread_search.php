...
if( $page['pagination.items'] ) {
	...
	<tr>
		<td>
			<a href="<?php echo $webimroot ?>/operator/threadprocessor.php?threadid=<?php echo $chatthread['threadid'] ?>" target="_blank" onclick="this.newWindow = window.open('<?php echo $webimroot ?>/operator/threadprocessor.php?threadid=<?php echo $chatthread['threadid'] ?>', '', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=720,height=520,resizable=1');this.newWindow.focus();this.newWindow.opener=window;return false;"><img src="<?php echo $webimroot ?>/images/tbliclread.gif" width="15" height="15" border="0" alt="<?php echo getlocal("page.analysis.search.head_messages") ?>"><?php echo topage(htmlspecialchars($chatthread['userName'])) ?></a>
		</td>
		<td>
