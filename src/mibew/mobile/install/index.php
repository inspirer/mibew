...
function check_mibewroot()
{
	...
	$newmibewroot = substr($application, 0, strrpos($applocation, '/mobile'));

	if ($applocation != $mibewroot . '/mobile') {
		$errors[] = "Please, check file ${applocation}/libs/config.php<br/>Wrong value of \$mibewroot variable, should be \"$newmibewroot\"";
		$mibewroot = $newmibewroot;
		return false;
	}
...
function check_files()
{
	...
	if ($fp === FALSE) {
		$errors[] = getlocal2("install.cannot_read", array("$mibewroot/mobile/install/package"));
...
function check_database($link)
{
	...
	if (mysql_select_db($mysqldb, $link)) {
		...
		$page['nextstepurl'] = "$mibewroot/mobile/install/dbperform.php?act=createdb";
	}
	return false;
}
...
function check_tables($link)
{
	...
	if ($curr_tables !== false) {
		...
		if (count($tocreate) == 0) {
			...
			$page['nextstepurl'] = "$mibewroot/mobile/install/dbperform.php?act=ct";
		}
	}
	return false;
}
...
function check_columns($link)
{
	...
	foreach ($dbtables as $id => $columns) {
		...
		if (count($tocreate) != 0) {
			...
			if (count($cannot_update) != 0) {
				...
				$page['nextstepurl'] = "$mibewroot/mobile/install/dbperform.php?act=dt";
	...
	if ($need_to_create_columns) {
		...
		$page['nextstepurl'] = "$mibewroot/mobile/install/dbperform.php?act=addcolumns";
...
function check_status()
{
	...
	if (!check_admin($link)) {
		...
		$page['nextnotice'] = getlocal2("installed.notice", array("${mibewroot}/mobile/install/"));
